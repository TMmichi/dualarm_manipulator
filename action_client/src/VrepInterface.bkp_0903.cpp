...
VrepInterface::VrepInterface(ros::NodeHandle& n) :
    ...
    while(clientID_ == -1 && ros::ok()) {
        clientID_ = simxStart((simxChar*)"127.0.0.1",19997,true,true,2000,5);
        if (clientID_ == -1) {
    ...
    success = success && initJoints(vrepFingerTipPrefix, urdfFingerTipPrefix,
            numFingerTipJoints_, jointState_, jointHandles_);
    if (!success) {
        ...
        success = success && initJoints(vrepFingerTipPrefix, urdfFingerTipPrefix,
                numFingerTipJoints_, jointState_, jointHandles_, 0);
        if (!success) {
